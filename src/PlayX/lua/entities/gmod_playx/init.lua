...
util.AddNetworkString("PlayXPlayPause")
util.AddNetworkString("PlayXSeek")
util.AddNetworkString("PlayXBegin")
util.AddNetworkString("PlayXEnd")
util.AddNetworkString("PlayXMetadataStd")
...
function ENT:PlayPause(paused)
    ...
    local filter = self:GetRecipientFilter()
    if not filter then return end
    self.Media.PausedAt = RealTime() - self.Media.StartTime

	net.Start("PlayXPlayPause")
...
function ENT:OpenMedia(provider, uri, start, forceLowFramerate, useJW, paused)
    ...
    -- Hook
    local r, err = self:MediaOpen(provider, uri, start, forceLowFramerate, useJW, result, paused)
    if r == false then
        return false, err
    ...
    self:BeginMedia(result.Handler, result.URI, start, result.ResumeSupported,
                    useLowFramerate, result.HandlerArgs, paused)
...
function ENT:Think()
    if self.Media then
        if self.Media.Paused and self.Media.PausedAt then
            self.Media.StartTime = RealTime() - self.Media.PausedAt
        end
    end
end
...
function ENT:SendStdMetadataMessage(ply)
	...
	net.Start("PlayXMetadataStd")
		net.WriteEntity(self)
		net.WriteString(self.Media.Title:gsub(1, 200))
	net.Send(filter)
end
